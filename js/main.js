...
document.querySelector(".search-icon").addEventListener("click", function showUpSearchBar() {
	document.querySelector(".search-enter-value").style.display = "flex";
	document.querySelector(".search-bar").value = "";
});
function hideSearchBar() {
	document.querySelector(".search-enter-value").style.display = "none";
...
// change transmission-image
	...
	var imgAlbum = ["url(../img/transition-img/img1.jpg)","url(../img/transition-img/img2.jpg)","url(../img/transition-img/img3.jpg)","url(../img/transition-img/img4.jpg)",];
	 document.querySelector(".transmission-image").transition = "0.3s";
	document.querySelector(".transmission-image").style.background = imgAlbum[imageN];
	document.querySelector(".transmission-image").style.backgroundSize = 'cover';
	imageN++;
...
function cofirmDiscount() {
	...
	if(checkInput.toUpperCase() === discountCode) {
		...
		useDiscountFixed = useDiscount.toFixed(2);
		document.querySelector('.total-amount__bottom #md4').innerHTML = useDiscountFixed;
	}
	if( document.querySelector(".enter-discount").value != "") {
		if(checkInput.toUpperCase() !== discountCode) {
...
//Trigger Button Click on Enter
 function triggerButton() {
	var input = document.querySelector(".enter-discount");
	input.addEventListener("keyup", function(event) {
	  if (event.keyCode === 13) {
	   event.preventDefault();
	   document.querySelector(".cofirm-code").click();
	  }
	});
}
triggerButton();
...
//show bought-list
document.querySelector(".bought-list svg").addEventListener("click",function showBoughtList() {
...
function filterItem() {
	var checkResult = 0;
	var input, filter,txtValue, itemChild, itemName,showResult;
	document.querySelector('.search-result').style.display = "grid";
	input = document.querySelector(".search-bar");
	showResult = document.querySelector('.search-result');
	showResult.innerHTML = "";
	filter = input.value.toUpperCase();
	itemChild = document.querySelectorAll(".item-child");
    for (var i = 0; i < itemChild.length; i++) {
		itemName = itemChild[i].querySelector(".item-name p");
		txtValue = itemName.textContent || itemName.innerText;
        if (txtValue.toUpperCase().indexOf(filter) > -1) {
			var getThisItem = document.querySelectorAll(".item-child")[i];
			console.log(getThisItem)
			var cloneIt = getThisItem.cloneNode(true);
			showResult.appendChild(cloneIt);
			addingId();
			checkResult++;
		}
	}
	if(checkResult === 0) {
		showResult.innerHTML = "No result ...";
	}
}